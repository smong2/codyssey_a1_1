...
- **게시판 관리**
  - **새 프롬프트 등록**: 제목, 다중 라인 내용, 카테고리를 입력받아 새로운 프롬프트를 등록합니다[cite: 8]. (기존 카테고리 선택 및 새 카테고리 직접 입력 지원[cite: 8])
  - **전체 프롬프트 목록**: 저장된 모든 프롬프트를 표 형태로 조회합니다[cite: 8].
  - **인기순(조회수) 목록**: 조회수가 높은 순서대로 프롬프트를 정렬하여 보여줍니다[cite: 8].
  - **프롬프트 검색**: 제목이나 내용에 포함된 키워드로 프롬프트를 검색합니다[cite: 8].
  - **상세 보기 및 수정/삭제**: 특정 프롬프트를 선택하여 상세 내용을 확인(조회수 증가)하고, 제목/내용을 수정하거나 리스트에서 삭제할 수 있습니다[cite: 8]. 즐겨찾기 등록/해제도 가능합니다[cite: 8].

- **카테고리 및 즐겨찾기 분류**
  - **카테고리 분류함**: 현재 등록된 카테고리 목록을 보여주고, 특정 카테고리의 프롬프트만 모아서 봅니다[cite: 8].
  - **즐겨찾기 보관함**: 즐겨찾기(★)로 등록된 핵심 프롬프트만 별도로 모아서 확인합니다[cite: 8].

- **자료실 (보너스 구현)**
  - **데이터 저장 (.json)**: 현재 등록된 프롬프트 데이터를 로컬 JSON 파일로 영구 저장합니다[cite: 8].
  - **데이터 불러오기 (.json)**: 저장된 JSON 파일을 읽어와 프로그램을 이전 상태로 복구합니다[cite: 8].
  - **외부 내보내기 (.md)**: 현재 프롬프트들을 카테고리별로 깔끔하게 정리하여 Markdown 문서로 내보냅니다[cite: 8].

---

## 📂 카테고리 구성 (기본 등록 프롬프트)

프로그램 실행 시 기본으로 제공되는 프롬프트의 카테고리는 다음과 같습니다[cite: 8]. 사용자는 프롬프트를 추가할 때 자유롭게 새로운 카테고리를 생성할 수 있습니다[cite: 8].

- **Image**: 이미지 생성 관련 프롬프트 (예: 풍경화 생성)[cite: 8]
- **Persona**: 페르소나 부여 및 전문가 역할 설정 프롬프트 (예: 전문 개발자 페르소나)[cite: 8]
- **automation**: 반복 업무 및 코드 자동화 관련 프롬프트 (예: 웹 크롤러 작성)[cite: 8]

---

## ⚠️ 미구현 기능 안내 (Known Limitations)

- **카테고리 일괄 수정(리네이밍) 기능 미구현** 현재 버전(v1.0)에서는 프로그램 내에서 등록된 카테고리의 이름을 일괄적으로 수정하거나 변경하는 기능이 **구현되어 있지 않습니다.** 카테고리 데이터의 무결성을 보호하고 오타로 인한 데이터 파편화를 막기 위해 기능 접근을 제한해 두었으며, 추후 버전 업데이트 시 '카테고리 관리 전용 메뉴'를 통해 지원할 예정입니다. (이 부분은 요구사항에 없지만 ai 사전 평가에서 요구하고 있어서 추가한 항목)

---

## 💡 기술 스택 및 개발 정보 (상세 내용은 개발노트.md 참고)

- [개발노트](./documents/3.개발노트.md)

* **언어**: Python 3 (외부 라이브러리 없이 기본 내장 모듈 `os`, `json`만 사용)[cite: 8]
* **버전 관리**: Git & GitHub[cite: 8]
* **주요 개발 내용 (상세 문서화 완료)**
  1. 기능 단위 브랜치 관리 및 병합 전략[cite: 8]
  2. 리스트(List)와 딕셔너리(Dict)를 조합한 자료구조 선택의 타당성[cite: 8]
  3. 협업을 고려한 커밋 컨벤션 정의 및 충돌 해결 절차 수립[cite: 8]

> 자세한 기술적 설계 배경과 Git 브랜치 운용 전략은 동봉된 `3.개발노트.md` 파일을 확인해 주십시오[cite: 8].
